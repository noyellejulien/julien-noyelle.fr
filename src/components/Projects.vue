...
export interface Project { id: number; test: string; title: string }
...
const getNextSiblings = (elem: ChildNode) => {
    var sibs = [];
    while (elem = elem.nextSibling as ChildNode) {
...
const getPrevSiblings = (elem: ChildNode) => {
    var sibs = [];
    while (elem = elem.previousSibling as ChildNode) {
        if (elem.nodeType === 3) continue; // text node
        sibs.push(elem);
    }
    return sibs;
...
const doThings = () => {
    const subcontainer = document.querySelector('.projects-container-subcontainer');

    if (subcontainer != null) {
        const prevElements = getPrevSiblings(subcontainer.parentElement as ChildNode);
        let start = 0;
        ...
        const nextElements = getNextSiblings(subcontainer.parentElement as ChildNode);
        start = 0;
...
<template >
    <div class="projects-container"
        :style="{ 'height': `${projectsHeight}px`, 'width': `${wWidth}px`, 'scroll-margin-top': `${headerHeight}px` }"
        id="projects">
        <div class="projects-container-subcontainer"
            :style="{ top: `${top}px`, transform: `translateX(${transform}px)`, height: `${height}px` }">
            <Project v-for="(project, index) in projects" :project="project" :wWidth="wWidth" :wHeight="wHeight" />
        </div>
    </div>
</template>
